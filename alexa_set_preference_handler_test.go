...
func TestAlexaSetPreferenceHandler(t *testing.T) {
	testTable := []struct {
		...
		{
			testName: "Returns proper response on successful save (default).",
			mockBusStopPreferenceStoreUserId:      "amzn1.ask.account.amzn1.ask.account.FAKEACCOUNT",
			mockBusStopPreferenceStorePrefName:    "default",
			mockBusStopPreferenceStoreStopCode:    "503471",
			mockBusStopPreferenceStoreError:       nil,
			...
			expectedOutputSpeechText:              "Saved 503471 as default.",
			expectedError:                         nil,
		},

		{
			testName: "Returns proper response on successful save (not default).",
			mockBusStopPreferenceStoreUserId:      "amzn1.ask.account.amzn1.ask.account.FAKEACCOUNT",
			mockBusStopPreferenceStorePrefName:    "brooklyn",
			mockBusStopPreferenceStoreStopCode:    "503478",
			mockBusStopPreferenceStoreError:       nil,
			mockBusStopPreferenceStoreCalledTimes: 1,
			alexaRequest:                          newFakeAlexaRequest("amzn1.ask.account.amzn1.ask.account.FAKEACCOUNT", "503478", "brooklyn"),
			expectedOutputSpeechText:              "Saved 503478 as brooklyn.",
			expectedError:                         nil,
		},
